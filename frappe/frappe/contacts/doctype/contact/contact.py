	...
	def on_update(self):
		for l in self.links:
			if l.link_doctype=="Customer":
				telefonos = frappe.db.sql(""" SELECT GROUP_CONCAT(phone SEPARATOR ' - ') AS telefonos
				FROM `tabContact Phone` where parent in (select parent from `tabDynamic Link` where 
				link_name like %(customer)s  and parenttype = 'Contact') """,{"customer":'%'+l.link_name+'%'})
				frappe.db.set_value("Customer",l.link_name,"todos_los_telefonos",telefonos[0][0])
