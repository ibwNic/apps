	...
	def execute(
		...
		*,
		...
		if (
			...
			if self.doctype == "Subscription Plan Detail" and ("Back Office" in roles or "Support Team" in roles):
				pass
			else:
		...
		if as_condition:
			...
			try:
				...
				if ("Vendedor Masivo" in user_roles or "Vendedor Corporativo" in user_roles) and frappe.session.user != "Administrator":
					...
					if sales_person: 
						...
						if self.doctype=="Lead":
							conditions = (f"sales_person='{sales_person}'")#" sales_person='" + sales_person +"'"
				...
				if "Tecnico" in user_roles and frappe.session.user != "Administrator":
					...
					tecnicos = [ tecnico[0] for tecnico in consulta ]
					tecnicos = str(tecnicos)
					tecnicos = tecnicos.replace("[","(").replace("]",")")
					if consulta: 
						if self.doctype=="Service Order":
