...
class Customer(TransactionBase):
	...
	def on_update(self):
		...
		frappe.msgprint(self.flags.is_new_doc)
		if self.lead_name:
			frappe.db.set_value("Lead",self.lead_name,"cliente",self.name)
...
def get_territory():
	...
	"""select name from `tabTerritory` where tipo_territorio= "Departamento" or name='Tipitapa'""",	)
	for r in result:
		name.append(r[0])
	return name
...
@frappe.whitelist()
def get_subscriptions(cliente):
	contratos = frappe.db.sql(""" select name from `tabSubscription` where party= %(cliente)s and workflow_state='Activo'; """,{"cliente":cliente})
	return [c[0] for c in contratos]

@frappe.whitelist()
def get_plans_customer(name):
	planes = frappe.db.sql(""" select spd.plan, spd.estado_plan, spd.parent as Contrato,  (select address_line1 from `tabAddress` where name=spd.direccion )as direccion, spd.cost as precio, spd.service_start, spd.currency,spd.name as PlanID
...
def get_work_orders(name):
	condicion = "%os%"
	ordenes = frappe.db.sql(""" select * from vw_ordenes_de_trabajo where tercero = %(name)s and orden like %(condicion)s order by fecha_solicitud desc; """, {"name":name, "condicion":condicion})
	return ordenes


@frappe.whitelist()
def get_bitacora_llamada(name):
	
	ordenes_c = frappe.db.sql(""" select name,fecha,motivo,gestion,tipo,numero,observaciones from `tabRegistro de llamada` where cliente = %(name)s order by fecha desc; """, {"name":name})
	return ordenes_c
...
def guardar_encuesta(**args):
	...
	encuesta_cliente = frappe.new_doc("Encuestas Respondidas")	
	encuesta_cliente.encuesta = args["id_encuesta"]
	encuesta_cliente.retroalimentacion = feedback.name
	encuesta_cliente.parent =  args["name"]
	encuesta_cliente.parenttype = args["doctype"]
	encuesta_cliente.parentfield = "encuestas_respondidas"
	encuesta_cliente.save(ignore_permissions=True)
	frappe.msgprint(frappe._('Encuesta {0} ha sido guardada').format(feedback.name))

@frappe.whitelist()
def filtrar_clientes_vendedor_masivo():
	try:
...
def actualizar_estado_customer(cliente):
	#estado de los clientes
	frappe.db.sql("""  CALL ActualizarClientes(); """)

def update_portafolios_customer():	
...
def actualizar_Portafolio_mas_factura():
	frappe.db.sql("""  CALL ActualizarPortafolioClientes(); """)
