...
class CambiodeRazonSocial(Document):
	def on_update(self):
		...
		frappe.db.sql("update `tabGestion` set razon_social = %(name)s where name = %(gestion)s;",{"name":self.name,"gestion":gestion.name})
...
@frappe.whitelist()
def obtener_planes_de_cliente(cliente,gestion):
	#frappe.msgprint(gestion)
	#time.sleep(1)
...
@frappe.whitelist()
def obtener_planes_de_cliente_after_save(name):
	crs = frappe.get_doc('Cambio de Razon Social',name)
	contratos = frappe.db.get_values("Subscription",{"party":crs.cliente, "workflow_state":"Activo"},'name')
	contratos = [c[0] for c in contratos]
	if len(contratos) == 0:
		frappe.msgprint("El cliente actual debe tener contratos Activos.")
		return
	planes = frappe.db.sql("""select name, plan, parent from `tabSubscription Plan Detail` where parent in %(parent)s and estado_plan = "Activo";""",{"parent":contratos})
	...
	for plan in planes:
		...
		item1.plan_name = plan[1]
		item1.contrato = plan[2]
	crs.save(ignore_permissions=True)
...
def crear_nuevo_contrato(name):
	try:
		...
		if not crs.nuevo_cliente or not crs.fecha_de_inicio_de_contrato or not crs.duracion_de_contrato or not crs.current_invoice_start or not crs.current_invoice_end or not crs.numero_de_nuevo_contrato:
			...
			return 
		if crs.nuevo_contrato:
			frappe.msgprint("Cambio de razon social ya fue creado")
			return
		...
		for plan in crs.cambiar_planes:
			if '+ TV' in plan.plan_name:
				...
				if 'GPON' in plan.plan_name:
					...
					hfc += 1
			if  'TV Combo GPON' in plan.plan_name or 'TV Combo HFC' in plan.plan_name :
				tv += 1		
				subsc2 = plan.contrato	
				if 'TV Combo GPON' in plan.plan_name:
					tv_gpon += 1
				elif 'TV Combo HFC' in plan.plan_name: 
					tv_hfc += 1			
		...
		frappe.db.sql("update `tabGestion` set workflow_state = 'Finalizado' where name = %(gestion)s;",{"gestion":crs.gestion})
			...
			old_plan_detail.update(
					{
						...
						"service_end": now(),
						"detalle_finalizado":'PLAN CERRADO POR CAMBIO DE RAZON SOCIAL',
					}
				)
			old_plan_detail.save()
			
			if 'TV Combo' in old_plan_detail.plan:
...
def Nuevos_contratos(crs):
	...
	new_suscripcion.campana = crs.campana
	...
	if new_suscripcion.campana:
		from erpnext.accounts.doctype.subscription.subscription import aplicar_promocion
		aplicar_promocion(new_suscripcion.campana,new_suscripcion.name)
	frappe.db.sql(""" update `tabCambio de Razon Social` set docstatus = 1 where name = %(name)s; """,{"name":crs.name})
	frappe.db.set_value("Subscription",new_suscripcion.name,"cambio_de_razon_social",crs.name)
	frappe.db.sql(""" update `tabCustomer` set estado_cliente = 'ACTIVO' where name = %(name)s; """,{"name":crs.nuevo_cliente})
	frappe.db.sql("update `tabGestion` set workflow_state = 'Finalizado', estado = 'Finalizado'  where name = %(gestion)s;",{"gestion":crs.gestion})
