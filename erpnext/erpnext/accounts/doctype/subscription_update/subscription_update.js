...
frappe.ui.form.on('Subscription Update', {
	refresh: function(frm) {	
		if(frm.doc.__islocal || frm.doc.nuevo_contrato !== undefined){
			frm.toggle_display("obtener_planes", false);
		}
		else{
			frm.toggle_display("obtener_planes", true);
		...
		if(frm.doc.contrato !== undefined && frm.doc.customer!== undefined){
			...
			frm.fields_dict.actualizar_planes_de_contrato.grid.get_field("nuevo_plan").get_query = function(doc, cdt, cdn){
				...
				let x = frappe.call({
					...
					"args":{
						'customer': frm.doc.customer,
						'desde_oportunidad':frm.doc.desde_oportunidad
						},
					"async": false, 
					callback: function(r){		
					}
				...
				if (plan && !x.responseJSON.message) {
					console.log("op1")
					if(plan.includes("+ TV")){
						return {
							filters: {
					...
					else if(plan === 'TV Combo GPON' || plan === 'TV Combo HFC' || plan.includes('GPON-TV-RESIDENCIAL SIN VELOCIDAD', 'ITV SIN VELOCIDAD')){
						return {
							filters: {
								name: ['in',['TV Combo GPON','TV Combo HFC','TV HFC']]
							}
						}
					}
					else{
					...
					return {
						filters: {
							name: ['in', x.responseJSON.message]
						}
					}
				}
				else{
...
frappe.ui.form.on("Subscription Update Planes", {
	nuevo_plan: function(frm,cdt,cdn) {
		...
		frappe.db.get_value("Subscription Plan", {"name": row.plan},"cost",function(res){ res.cost; })
		.then(r =>{
			
			let rest=r.message;
			frappe.model.set_value(cdt, cdn, 'coston', r.message.cost); }
		) 
