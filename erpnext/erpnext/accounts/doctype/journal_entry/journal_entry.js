...
frappe.ui.form.on("Journal Entry", {
	...
	refresh: function(frm) {
		...
		var series = {
			...
			"hegarcia@ibw.com": "K-",
			"jacquelines.martinez@ibw.com": "J-",
			"glendy.garcia@ibw.com": "G-",
			"jaime.lopez@ibw.com":"F-",
			"mariling.medina@ibw.com":"D-",
			"mbenavides@ibw.com":"K-",
			"elvis.martinez@ibw.com":"E-",
			//Cobranza
			'dmorales@ibw.com': "JV-",
			'maria.jose@ibw.com': "JV-",
			'elopez@ibw.com.ni': "JV-",
			'cvelasquez@ibw.com.ni': "JV-", 	
			// 'octavio.aguirre@ibw.com':"O-"
		};

		// if (series[frappe.user.name]){
		...
		// 	// frm.toggle_enable('voucher_type', false);
		// }
		series[frappe.user.name] && frm.doc.__islocal && frm.set_value('naming_series', series[frappe.user.name]);
		// frm.toggle_enable('naming_series', false);
		...
		//#region Pagos Caja
		cur_frm.cscript.make_customer_payment = function(frm,currentUser,user){
						...
						'callback': function(res){
							...
							if (res.message) {
								if (res.message.accounts.length && !res.message.messages.length){
									...
									if (currentUser === 'Administrador Caja'){
										cur_frm.set_value("tipo_de_pago", 'DepositoBanco');
										cur_frm.set_value("no_registrar_cierre",1);
									}
			...
			d.get_primary_btn().text('Crear Pago').off('click').on('click', function(){
				...
				// if(user === 'maria.juarez@ibw.com'){
				// 	var values = d.get_values();
				// 	ValidarCamposDigitados(pm_args,tipopago,monedaNIO,monedaUSD,values);
				// }
				// else if(user === 'jaime.lopez@ibw.com'){
				// 	for(let i = 0; i < payments.length; i++){
				// 		if (payments[i]['tipo_de_pago'] === "Telepago" || payments[i]['tipo_de_pago'] === "Cheque"){
				// 			var values = d.get_values();
				// 			ValidarCamposDigitados(pm_args,tipopago,monedaNIO,monedaUSD,values);
				// 			break;
				// 		}
				...
				// if(user === 'jaime.lopez@ibw.com' || user === 'mariling.medina@ibw.com'){
				// 	for(let i = 0; i < payments.length; i++){
				...
				for(let i = 0; i < payments.length; i++){
					if (payments.length === 1){
						if (payments[i]['tipo_de_pago'] === "Telepago" || payments[i]['tipo_de_pago'] === 'Cheque'){
							if(user === 'jaime.lopez@ibw.com'){
								telepago = true;
								break;
							}
							
						}
						if (payments[i]['tipo_de_pago'] === 'Efectivo'){
							if(user === 'jaime.lopez@ibw.com'){
								telepago = true;
								break;
							}
				...
				if(telepago === true){
					...
					// PASE PARA PROCEDER CON LOS PAGOS USD CUANDO LOS MONTOS NO SON IGUALES
		...
		cur_frm.cscript.CrearAnticipos = function(frm,currentUser,rol){
						...
						'callback': function(res){
							if (res.message) {
								if (res.message.accounts.length && !res.message.messages.length){
									...
									cur_frm.set_value("regnumber", d.get_value("customer"));
									if(d.get_value("Colector")){
										cur_frm.set_value("collector", d.get_value("Colector"));
										cur_frm.set_value("no_registrar_cierre",1);
									}
									if(rol === 'Administrador Caja'){
										cur_frm.set_value("no_registrar_cierre",1);
									}
			...
			function toggle_actionCambio(fields, wrapper){
				...
				function create_payments_areaAnticipo(currentUser,rol){
					if (rol == 'Administrador Caja'){	
						var modoPago = ['','BANPRO','BAC','Bancentro','BDF','Efectivo','Cheque'];                        
		
					}else if(currentUser === 'jaime.lopez@ibw.com'){
						var modoPago = ['','Cheque','Efectivo','Tarjetas de credito','Telepago'];
					}
					else{
						var modoPago = ['','Cheque','Efectivo','Tarjetas de credito']; 
					}
					var mode_of_payment_field = frappe.ui.form.make_control({
						df: {
							fieldtype: 'Select',
										...
										frappe.call({
											...
											'callback': function(res){
												...
												if (res.message == "Pendiente"){
													...
													create_payments_areaAnticipo(currentUser,rol);
													...
													frappe.call({
														...
														'callback': function(res){
															...
															if (res.message[0][0] === null && res.message[0][1] === null){
																frappe.msgprint({
																	title: __('Advertencia'),
																	indicator: 'red',
																	message: __('Lo sentimos el cliente tiene sus servicios inactivos!')
																});
																return 0
															}else if(res.message[0][1] == "USD"){
			...
			d.show();
			if (currentUser != 'maria.juarez@ibw.com' && currentUser != 'Administrator'){
				d.set_df_property('Colector', 'hidden', 1);
			}
			...
			d.get_primary_btn().text('Crear Anticipo').off('click').on('click', function(){
				...
				if (total1 && total1 >= 200){
					Monto_DigitadoNIO = true;
					
				}

				if (total2 && total2 >= flt(200 / d.get_value('conversion_rate'),2)){
					Monto_DigitadoUSD = true;
				}
				...
				if(rol === 'Administrador Caja'){
					...
					if (Monto_DigitadoNIO == true && Monto_DigitadoUSD == true){
						...
						frappe.msgprint({
							title: __('Advertencia'),
							indicator: 'red',
							message: __('Lo sentimos no puede realizar Anticipo')
						});
						// frappe.msgprint("No es igual a los montos totales!");
					};
				}
		...
		cur_frm.cscript.AplicarAnticipos = function(frm){
			...
			function Crear_Deposito(pm_args,values){
				...
				frappe.call({
					...
					'callback': function (r) {
						...
						console.log(r.message)
						if (r.message){
							r.message.accounts.forEach(function(c){
								frm.add_child("accounts",c);
							});
							// cur_frm.set_value("posting_date", pm_args.fecha);
							cur_frm.set_value("multi_currency", 1);
							cur_frm.set_value("tipo_de_pago", 'Aplicar Anticipo');
							cur_frm.set_value("codigoanticipo", codigoanticipo);
							cur_frm.set_value("regnumber", d.get_value('customer'));
							cur_frm.set_value("no_registrar_cierre", 1);
							cur_frm.refresh_fields()
							cur_frm.trigger("validate");
			...
			function SumaFormaPagos() {
				...
				function mostrar_Campos_de_Anticipos(){
					var args = {}
					
					args['Customer'] = d.get_value('customer');
					frappe.call({
						...
						callback: function(res){
							...
							if(res.message.length == 0){
								frappe.msgprint({
									title: __('Advertencia'),
									indicator: 'green',
									message: __('No tiene Anticipos registrados!')
								});
								d.set_value("customer",null);
							}

							if (res.message){
								d.set_value("Meses",res.message.Meses_Anticipos);
								d.set_value("MontoNIO",res.message.MontoNIO);
								d.set_value("moneda",res.message.moneda);
								d.set_value("conversion_rate",res.message.tc);
								codigoanticipo = res.message.Name_Anticipo;
								// d.set_value("MontoUSD",res.message.MontoUSD);
							}
							// if (res && res.message) {
							// 	if (res.message.deudas.length) {
							// 		d.fields_dict.debits_wrapper.$wrapper.html(frappe.render(
			...
			d.get_primary_btn().text('Aplicar Deposito').off('click').on('click', function(){
				...
				// 	return 0
				// }
				console.log(d.get_value('moneda'),d.get_value('Monto'))
				// pm_args.pagos = payments;
				console.log(pm_args.deudas.length,pm_args.pagos.length);
				if (pm_args.deudas.length != pm_args.pagos.length){
					...
					if (d.get_value('moneda') == 'USD'){
						if( totals[0]['MontoUSD'] > 0 && totals[0]['MontoUSD']){
							pm_args.pagos = payments;
					...
					}else if (d.get_value('moneda') == 'NIO'){
						if(totals[0]['MontoNIO'] <= d.get_value('MontoNIO')){
							console.log('Entra');
							pm_args.pagos = payments;
							// pm_args.ID_pago_ZZ = frm.doc.name; 
							// pm_args['cuentaBanco'] = d.get_value('cuenta');
		...
		cur_frm.cscript.CrearDepositoGarantia = function(frm,currentUser){
			function ValidarCamposDigitados (pm_args,tipopago,monedaNIO,monedaUSD,values){
				for(let i = 0; i < pm_args.pagos.length; i++){
					if (pm_args.pagos[i]['tipo_de_pago'] == "Efectivo"){
						console.log("Entra");
		...
		//#region Pagos Centeno y Pagos Parciales
		cur_frm.cscript.CrearPagoCenteno_o_Parcial = function(frm,Centeno,Name_Pago,rol,currentUser){
			...
			function aplicar_Pagos (values){
				...
				console.log(Cambios);
				frappe.call({
					'method': 'erpnext.api.aplicar_pago_Centeno_Parcial',
					// 'method': 'erpnext.api.aplicar_pago',
					'args': pm_args,
					'callback': function(res){
						console.log(res.message);
						if (res.message) {
							if (res.message.accounts.length && !res.message.messages.length){
								...
								// if(Centeno === 0){
								// 	cur_frm.set_value("centeno", Centeno);
								// }
			...
			frappe.call({
				...
				callback: function(res){
					...
					if (res && res.message) {
						...
						minimal_credit_amount = res.message.monto_minimo_credito;
						setTimeout(function(){ ignore_event = false; }, 500);
						create_payments_area(res.message,rol);
					}
				}
			});
		}
		...
		// var roll = rol;
		function create_payments_area(data,rol){
			// options: ['', 'Efectivo','Tarjetas de credito'],
			if (rol == 'Administrador Caja'){	
				var modoPago = [null].concat(data.tipos_de_pago);                        

			}else{
				var modoPago = ['','Efectivo','Cheque','Tarjetas de credito']; 
			}
			var mode_of_payment_field = frappe.ui.form.make_control({
				df: {
					fieldtype: 'Select',
		...
		d.show();
		// d.get_field("posting_date").toggle_enable(true)
		// d.toggle_enable("posting_date",true)
		d.$wrapper.find(".modal-dialog").css({"width": "100%","left": "-10%","top": "10%"});
		d.$wrapper.find(".modal-content").css({"width": "160%"});
		...
		d.get_primary_btn().text('Crear Pago').off('click').on('click', function(){
			...
			//Pagos con validaciones menor a la factura
			// if (total1 >= 1  && total1 <= montos[0] && total2 >=1 && total2 <= montos[1] && total2 >=1 && total2  <= montos[2]){

			//FALTA VALIDAR EL MONTO 
			if(rol === 'Administrador Caja' || currentUser === 'mariling.medina@ibw.com'){
				var values = d.get_values();
				ValidarCamposDigitados(pm_args,tipopago,monedaNIO,monedaUSD,values);
			} else {
				if (total1 >= 1  && total1 <= montos[0] && total2 >=1 && total2  <= montos[2]){
					var values = d.get_values();
					ValidarCamposDigitados(pm_args,tipopago,monedaNIO,monedaUSD,values);
					// frappe.msgprint("Pasa al pago!");
				} else {
					frappe.msgprint({
						title: __('Advertencia'),
						indicator: 'red',
						message: __('El monto digitado no debe ser mayor al monto de la factura!')
					});
					// frappe.msgprint("No es igual a los montos totales!");
				};
			}
				...
				frappe.call({
					...
					'callback': function (r) {
						...
						if (r.message){
							...
							cur_frm.set_value("codigodeposito", codigoDeposito);
							cur_frm.set_value("regnumber", d.get_value('customer'));
							cur_frm.set_value("no_registrar_cierre", 1);
							cur_frm.refresh_fields()
							cur_frm.trigger("validate");
					...
					frappe.call({
						...
						callback: function(res){
							...
							if (res.message){
								...
								d.set_value("moneda",res.message.moneda);
								d.set_value("conversion_rate",res.message.tc);
								codigoDeposito = res.message.Name_Deposito;
								// d.set_value("MontoUSD",res.message.MontoUSD);
							}
							// if (res && res.message) {
							// 	if (res.message.deudas.length) {
		...
		cur_frm.cscript.ArregloDePago = function(frm,currentUser){
            ...
            function aplicar_Pagos(values){
					...
					pm_args._ui_=true;
					pm_args.CodigoArreglo=codigoArreglo;
					// console.log(codigoArreglo);
					console.log(CuotasTbl);
					console.log('Requisición Aplicar Arreglo de Pago');
					console.log(Object.keys(pm_args).map(function(k) { return k + ': ' + JSON.stringify(pm_args[k]) }).join('\n'));
					// console.log(Cambios);
					frappe.call({
						'method': 'erpnext.api.Aplicar_Arreglo_de_Pago',
						'args': pm_args,
						'callback': function(res){
							console.log(res.message);
							if (res.message) {
								if (res.message.accounts.length && !res.message.messages.length){
									...
									CuotasTbl.DetalleCuotas.forEach(function(c){
										frm.add_child("referencia_cuotas", c);
									});
									cur_frm.refresh_fields()
									cur_frm.set_value("posting_date", pm_args.fecha);
									cur_frm.set_value("multi_currency", 1);
									cur_frm.set_value("regnumber", d.get_value("customer"));
									cur_frm.set_value("codarreglopago",codigoArreglo)
									cur_frm.set_value('aplico_arreglo_de_pago',1);
									// cur_frm.set_value("monto_recibido_dolares", fields.montoUSD.get_value());
									// cur_frm.set_value("monto_recibido_cordobas", fields.montoNIO.get_value());
									if (fields.montoUSD.get_value() == null && fields.montoNIO.get_value() == null) {
										cur_frm.set_df_property('monto_recibido_dolares', 'read_only', 1)
			...
			var d,
			   ...
			   <thead>
				 <tr>
				   <tr>
					   ...
					   <th class="text-right">Monto C</th>
					   <th class="text-right">Monto C$ TC {{data.tc}}</th>
				   </tr>
				 </tr>
			   </thead>
			   <tbody>
			   {% for (var row of data[dc]) { %}
					<tr>
							<td><input type="checkbox" class="link_name" value="{{row.Abono}}" data-doctype="{{ row.doctype }}" data-monto="{{ row.Monto }}" /></td>
							<td>{{ frappe.datetime.str_to_user(row.Fecha) }}</td>
							<td class="text-right">{{ row.Abono }}</td>
							<td class="text-right">{{ format_currency(row.Monto, "USD", 2) }}</td>
							<td class="text-right">{{ format_currency(flt(row.Monto) * flt(data.tc), "NIO", 2) }}</td>
					</tr>
				 {% }%}
			   </tbody>
			...
			tmp_pm_table = `<table class="table table-condensed table-bordered">
			   ...
			   <tbody>
				 ...
				 <tr>
				   <td>{{ row.tipo_de_pago }}</th>
				   <td>{{ {"C$": "C$", "$": "$"}[row.moneda] }}</th>
				   <td class="text-right">{{ format_currency(row.moneda === "$" ? flt(row.monto) : flt(row.monto) / flt(tc,2), "USD", 2) }}</th>
				   <td class="text-right">{{ format_currency(row.moneda === "$" ? flt(row.monto) * flt(tc) : flt(row.monto), "NIO", 2) }}</th>
				   <td><button class="btn btn-danger btn-xs remove" data-idx="{{ i }}"><span class="icon icon-remove">
				 	<svg class="icon icon-sm" style>
						<use class href="#icon-delete"></use>
				 ...
				 {% totals.pm_total_usd += (row.moneda === "$" ? flt(row.monto) : flt(row.monto) / flt(tc,2)) %}
				 {% totals.pm_total_nio += (row.moneda === "$" ? flt(row.monto * tc,2) : flt(row.monto)) %}
				 {% } %}
			   </tbody>
			   <tbody>
				 <tr>
			...
			tmp_tt_table = `<table class="table table-condensed table-bordered">
				<thead>
					<tr>
						<th colspan="2">&nbsp;</th>
						<th class="text-right">Monto $</th>
						<th class="text-right">Monto C$ TC</th>
					</tr>
				</thead>
				<tbody>
					{% var cls = {"=": "", "+": "text-success", "-": "text-danger"} %}
			...
			function set_sum_on_check(wrapper, dc, data){
				wrapper.find('input[type="checkbox"]').on('change', function()  {
					...
					// var montos = [];
					pm_args[dc] = [];
					CuotasTbl['DetalleCuotas'] = [];
					wrapper.find('input[type="checkbox"]:checked').each(function(){
						pm_args[dc].push({'link_doctype': $(this).data('doctype'), 'link_name': $(this).val()});
						CuotasTbl['DetalleCuotas'].push({'cuota': $(this).val(),'monto': $(this).data('monto')});
						frappe.utils.filter_dict(data['cuotas'], {'Abono': $(this).val()}).forEach(function(d){
							amounts.usd += d.Monto;
							amounts.nio += d.Monto * tc;
							// amounts.actual_nio += d.actual.nio;
							// amounts.diff += d.actual.diferencial;
						});
					});
			...
			function get_outstanding_details(){
				...
				console.log('Requisición Consulta Arreglo de Pago');
				console.log(Object.keys(args).map(function(k) { return k + ': '+ JSON.stringify(args[k]) }).join('\n'));
				// console.log(args);
				frappe.call({
					'method': 'erpnext.api.Consultar_Arreglo_de_Pago',
					'args': args,
					'freeze': true,
					'freeze_message': 'Buscando informaciones!',
					callback: function(res){
						console.log(res.message);
						if(res.message.cuotas.length == 0){
							frappe.msgprint({
								title: __('Advertencia'),
								indicator: 'red',
								message: __('El cliente no tiene Arreglos de Pagos!')
							});
							d.set_value("customer",null);
						}
						if (res && res.message) {
							if (res.message.cuotas.length) {
								// let cuotas =  res.message;
								d.fields_dict.debits_wrapper.$wrapper.html(frappe.render(
									tmp_dc_table, {data: res.message, dc: "cuotas"}
								));
								// const data = document.querySelector('#debits_wrapper')
								// data.innerHTML = frappe.render(tmp_dc_table, {'data': 'Stt'});

								set_sum_on_check(d.fields_dict.debits_wrapper.$wrapper, 'deudas', res.message);
								TCBAnco(res.message.lafise);
								d.set_value('conversion_rate',res.message.tc);
								tc = res.message.tc;
								TituloName(res.message.cliente);
								// frappe.db.get_value("Currency Exchange", {"date": '25-04-2023'},"lafise",function(res){
								// 	res.lafise;
								// 	console.log(res.lafise);
									
								// })
								
								//REVISAR A LA HORA DE AGREGAR FORMA DE PAGO
								// frappe.db.get_value("Currency Exchange", {"date": '25-04-2023'},"paralela",function(res){
								// 	res.paralela;
								// 	console.log(res.paralela);
								// 	d.set_value('conversion_rate',res.paralela);
								// })
								
								if (res.message.codigoarreglo){
									codigoArreglo = res.message.codigoarreglo;
								}
								// frappe.utils.filter_dict(res.message).forEach(function(d){
								// 	codigoArreglo = d.CodigoArreglo;
								// });
								
								

								// frappe.db.get_value('Currency Exchange',{'date':today()}, 'lafise')
							...
							// minimal_credit_amount = res.message.monto_minimo_credito;
							setTimeout(function(){ ignore_event = false; }, 500);
							create_payments_area(currentUser);
						}
					}
				});
			}
			...
			function create_payments_area(currentUser){
				if(currentUser === 'jaime.lopez@ibw.com'){
					var modoPago = ['','Cheque','Efectivo','Tarjetas de credito','Telepago'];
				}
				else{
					var modoPago = ['','Cheque','Efectivo','Tarjetas de credito']; 
			...
			d.get_primary_btn().text('Crear Pago').off('click').on('click', function(){
				// console.log(totals[1]);
				// Validacion de montos
				var total2 = totals[1]['amount_nioF'];
				var total1 = totals[1]['amount_usd'];
				var tipopago = 0;
				var Monto_DigitadoNIO = false;
				var Monto_DigitadoUSD = false;
				...
				for(let i = 0; i < pm_args.pagos.length; i++){
					if (pm_args.pagos[i]['tipo_de_pago'] == "Telepago"){
						telepago = true
					}
				}

				if(currentUser === 'jaime.lopez@ibw.com' && telepago === true){
					var values = d.get_values();
					ValidarCamposDigitados(pm_args,tipopago,monedaNIO,monedaUSD,values);
				}
				else if (Monto_DigitadoNIO == true && Monto_DigitadoUSD == true){
		...
		//#region Esfrozar cancelacion
		cur_frm.cscript.AplicarCancelacion = function(frm){
			// console.log(frm.doc.name)
			frappe.call({
				args: {
					"name": frm.doc.name
				},
				method: "erpnext.accounts.doctype.journal_entry.journal_entry.unlink_pago_cierre_BTN",
			});
		}
		//#endregion
	    ...
	    if (frm.doc.docstatus === 0){
            frm.add_custom_button('Crear Pago', function(){
				...
				// var current = rol;
				// console.log(current)
                cur_frm.cscript.make_customer_payment(frm,rol,user);
            }, __("Caja"));
        }
		...
		if (frm.doc.docstatus === 0){
            frm.add_custom_button('Crear Anticipo', function(){
                ...
                cur_frm.cscript.CrearAnticipos(frm,currentUser,rol);
            }, __("Caja"));
        }
		...
		if (frm.doc.docstatus === 0){
            frm.add_custom_button('Crear Deposito en Garantia', function(){
				var currentUser = frappe.session.user;
				console.log(currentUser)
                cur_frm.cscript.CrearDepositoGarantia(frm,currentUser);
            }, __("Caja"));
        }

		if (frm.doc.docstatus === 0){
            frm.add_custom_button('Pago Centeno', function(){
				var currentUser = frappe.session.user;
                cur_frm.cscript.CrearPagoCenteno_o_Parcial(frm, 1,'Aplicar Pago Centeno',rol,currentUser);
            }, __("Caja"));
        }

		if (frm.doc.docstatus === 0){
			frm.add_custom_button('Arreglo de Pago', function(){
				var currentUser = frappe.session.user;
				cur_frm.cscript.ArregloDePago(frm,currentUser);
			},__("Caja"));
		}
		...
		frappe.call({
			"method": "erpnext.crm.doctype.opportunity.opportunity.consultar_rol",
				...
				if((r.message.includes("Administrador Caja"))){
					...
					if (frm.doc.docstatus === 1){
						frm.add_custom_button('Esforzar Cancelacion', function(){
							cur_frm.cscript.AplicarCancelacion(frm);
						}, __("Finanzas"));
					}
					...
					if (frm.doc.docstatus === 0){
						frm.add_custom_button('Pago Parcial', function(){
							cur_frm.cscript.CrearPagoCenteno_o_Parcial(frm, 0,'Aplicar Pago Parcial',rol);
						});
					}

					// if (frm.doc.docstatus === 0){
					// 	frm.add_custom_button('Arreglo de Pago', function(){
					// 		cur_frm.cscript.ArregloDePago(frm);
					// 	});
					// }
				}
			}
		...
		frappe.call({
			"method": "erpnext.accounts.doctype.journal_entry.journal_entry.Get_socursal",
			'args': {'user':user},
		}).then(r =>{

			console.log(user,r.message.length)
			const serie = [];

			if (user == 'Administrator' || rol.includes('Administrador Caja') || rol.includes('Cobranza')){
				
			}else{

				if (r.message){
					for (var i = 0; i < r.message.length; i++){
						if (r.message[i] == 'Tipitapa'){
							serie.push('J-','J-Manual-');
						}
						else if (r.message[i] == 'Masaya'){
							serie.push('I-','I-Manual-');
						}
						else if (r.message[i] == 'Leon'){
							serie.push('D-','D-Manual-','D-Centeno-');
						}
						else if (r.message[i] == 'Carazo'){
							serie.push('K-','K-Manual-','K-Centeno-');
						}
						else if (r.message[i] == 'Matagalpa'){
							serie.push('E-','E-Manual-');
						}
						else if (r.message[i] == 'Managua'){
							serie.push('F-','F-Manual-');
						}
					}
	
					set_field_options('naming_series',serie);
