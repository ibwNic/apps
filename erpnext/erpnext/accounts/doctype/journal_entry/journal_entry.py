...
from frappe.utils import cint, cstr, flt, fmt_money, formatdate, get_link_to_form, nowdate, today
...
def ChecarAnticipo(Customer):
	Fact = frappe.db.sql(
		"""select  name,nombre_del_documento,cliente,monto from `tabAnticipos` where cliente=%(Customer)s and aplicado = 0;""",
		{"Customer": Customer},
	)
	...
	if Fact:
		...
		# No tiene registrados anticipados
		return 0
	# return "ERROR"
...
def ChecarDeposito(Customer):
	Fact = frappe.db.sql(
		"""select  name,nombre_del_documento,cliente,monto from `tabDeposito en Garantia` where cliente=%(Customer)s and aplicado = 0;""",
		{"Customer": Customer},
	)
