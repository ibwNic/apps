...
frappe.ui.form.on('Nota de Credito', {
	refresh: function(frm) {
		...
		cur_frm.cscript.AplicarNotaCredito = function(frm){
			...
			function Crear_NotaCredito(pm_args,values){
				...
				console.log(Object.keys(pm_args).map(function(k) { return k + ': ' + JSON.stringify(pm_args[k]) }).join('\n'));
					// AQUI
				frappe.call({
					'method': "erpnext.api.Aplicar_Nota_Credito",
					'args': pm_args,
					'callback': function (r) {
			...
			function SumaFormaPagos() {
							...
							for (let i=0;i < FacturaYMonto.length; i++){
								if (FacturaYMonto[i].link_name === mode_of_payment_field.get_value()){
									if(frm.doc.moneda === 'USD'){
										if (amount_field.get_value() > flt(FacturaYMonto[i].MontoUSD,2)){
											frappe.msgprint({
												title: __('Error'),
												indicator: 'red',
				...
				d = new frappe.ui.Dialog({
					title: __("Aplicar Nota de Credito"),
					fields: [
						{
							fieldtype: "HTML",
			...
			d.set_value('customer',frm.doc.regnumber)
			...
			d.get_primary_btn().text('Aplicar Nota de Credito').off('click').on('click', function(){
				...
				if (pm_args.deudas.length != pm_args.pagos.length){
					...
					if (frm.doc.moneda == 'USD'){
						if( flt(totals[0]['MontoUSD'],2) > 0 && flt(totals[0]['MontoUSD'],2) <= frm.doc.monto){
							pm_args.pagos = payments;
		...
		if (frm.doc.workflow_state === 'Aprobado'){
...
frappe.ui.form.on("Nota de Credito", "fecha", function(frm){
...
frappe.ui.form.on("Nota de Credito", "regnumber", function(frm){
	frappe.db.get_value("Customer", {"name": frm.doc.regnumber},"customer_name",function(res){
		frm.set_value('nombre',res.customer_name)
	})
});

frappe.ui.form.on("Nota de Credito", "regnumber", function(frm){
	frappe.db.get_value("Customer", {"name": frm.doc.regnumber},"customer_group",function(res){
		frm.set_value('tipo_cliente',res.customer_group)
	})
});

frappe.ui.form.on("Nota de Credito", "regnumber", function(frm){
	frappe.db.get_value("Customer", {"name": frm.doc.regnumber},"item_group",function(res){
		frm.set_value('portafolio',res.item_group)
	})
});
