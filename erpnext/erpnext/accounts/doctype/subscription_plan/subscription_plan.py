...
def get_plan_rate_NEW(cost, start_date=None, end_date=None,periodo=None):
	...
	if int(periodo)>1:
		...
		diff_T= flt(date_diff(end_date,get_first_day(start_date)))+1
		
	diff_P=flt(date_diff(end_date,start_date)) + 1
	price=flt(cost/diff_T)
	price_P=flt(price*diff_P)*int(periodo)
	# print(f"el diff_T es: {diff_T}")
	# print(f"el diff_P es: {diff_P}")
	# print(f"el precio total es: {price_P}")
