...
def iterador_de_pbs(names, ruta, name):
	directorio = "/data/frappe-bench/sites/formatos/facturaciones/" + name
	try:
		os.mkdir(directorio)
	except OSError:
		pass
	...
	for name in names:
		try:
			...
			customer_name = customer_name.replace("  "," ").replace(" NULL","").replace("/","")
			...
			customer_info = frappe.db.sql(""" SELECT cedula, ruc, address_line1, municipio, territory FROM `tabCustomer` where name =  %(customer)s  """, {"customer":customer})
			...
			saldo_pendiente = frappe.db.sql(""" select sum(outstanding_amount) from `tabSales Invoice` where customer = %(customer)s and docstatus = 1 """, {"customer":customer})
			try:	
				saldo_pendiente = "{:.2f}".format(float(saldo_pendiente[0][0]))
			except:
				saldo_pendiente = 0.00
			collector = frappe.db.sql(""" select name, telefono from vista_colector where link_name = %(customer)s limit 1;""", {"customer":customer})

			planes = frappe.db.sql("""select * from vista_planes_facturas where name = %(name)s and descripcion not in ('GponTv22','ITV22');""",{"name": name})
			# edit file
			f.write(f"101|{name}|{customer_name}{cedula}{ruc}|{phone}{direccion}|{municipio}|{customer}|{mes}|{posting_date}|{due_date}|{saldo_pendiente}\n")
			f.write("2\n")
			f.write("3\n")

			paralela=''
			for plan in range(len(planes)):	
				# fullstring = planes[plan][4]
				# first_substring = 'HFC23-'
				# second_substring = 'GPON22'
				nombre = str(planes[plan][3])
				...
				if currency=="USD" and currency_item=="NIO":
					amount = "{:.2f}".format(float(planes[plan][5])/paralela)
				elif currency=="NIO" and currency_item=="USD":
					amount = "{:.2f}".format(float(planes[plan][5])*paralela)
				else:
					amount = "{:.2f}".format(float(planes[plan][5]))
				# if search(first_substring, fullstring) or search(second_substring, fullstring):			
				# 	f.write(f"    1| {nombre} De {emision} A {facturacion}|{moneda} {float(amount) + 5.00}|{moneda} {float(amount) + 5.00}\n")

				# else:
				f.write(f"    1| {nombre} De {emision} A {facturacion}|{moneda} {amount}|{moneda} {amount}\n") 
