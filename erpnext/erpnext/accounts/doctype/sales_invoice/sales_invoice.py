...
class SalesInvoice(SellingController):
	...
	def on_submit(self):
		if self.oportunidad:
			frappe.db.sql(""" update `tabOpportunity` set docstatus=1, status = 'Closed' where name = %(name)s ;""",{"name":self.oportunidad})
	...
	def unlink_envio_factura(self):
		frappe.db.sql(
			""" update `tabDetalle Envio de Factura`
			set factura = null where factura = %(p2)s""",
			{"p2":self.name},
		)
...
@frappe.whitelist()
def factura_venta_de_equipos(oportunidad):
	opp = frappe.get_doc("Opportunity",oportunidad)
	items = frappe.db.get_values("Ventas de Equipos",{"parent":oportunidad},["producto","descripcion","qty","precio_unitario","precio_total","portafolio_ve"])
	...
	for m in items:
		...
		item1.income_account = "4.10.005-EQUIPOS - NI"
		total = total + float(m[4])
...
def iterador_de_pbs(names, ruta, name):
	...
	for name in names:
		try:
			# sales invoice
			info = frappe.get_doc("Sales Invoice",name)
			customer = str(info.customer)

			deudores = frappe.db.get_value("Party Account", customer, "account")
			
			customer_name = str(info.customer_name)
			verf = list(customer_name)
			if verf.pop() == " ":
				customer_name = customer_name[:-1]
			customer_name = customer_name.replace("  "," ").replace(" NULL","").replace("/","").replace("	"," ").replace("\n","")
			posting_date = str(info.posting_date)
			posting_date = posting_date.replace("-","/")
			emision= frappe.utils.get_datetime(info.posting_date).strftime("%b %d, %y") 
			facturacion = frappe.utils.get_datetime(frappe.utils.get_last_day(info.posting_date)).strftime("%b %d, %y")
			due_date = str(info.due_date)
			due_date = due_date.replace("-","/")
			mes = frappe.utils.formatdate(info.posting_date, "MMMM")
			mes = mes.capitalize()
			
			total = "{:.2f}".format(float(info.total))
			iva = "{:.2f}".format(float(info.total_taxes_and_charges))
			grand_total = "{:.2f}".format(float(info.grand_total))
			base_grand_total = "{:.2f}".format(float(info.base_grand_total))
			...
			# customer querys
			phone_query = frappe.db.sql(""" 
				select phone from `tabContact Phone` where parent in (select link_title from `tabDynamic Link` where 
				link_name = %(customer)s and parenttype = 'Contact') limit 1; """, {"customer" : customer} )

			try:
				phone = phone_query[0][0] + "|"
			except:
				phone = "|"   
			customer_info = frappe.db.sql(""" SELECT cedula, ruc, address_line1, municipio, territory,factura_cordoba FROM `tabCustomer` where name =  %(customer)s  """, {"customer":customer})
			cedula = customer_info[0][0]
			ruc = customer_info[0][1]
			...
			if ruc:
				...
				ruc = ""
			try:
				direccion = customer_info[0][2].replace("  "," ").replace(" NULL","").replace("/","").replace("	"," ").replace("\n","")
			except:
				direccion = "	DIRECCION 	"
			try:
				municipio = str(customer_info[0][3])
				municipio = municipio.replace(" Mun","")
				municipio = municipio.upper()
			except:
				municipio = "MUNICIPIO"

			
			saldo_pendiente = frappe.db.sql(""" select sum(outstanding_amount) from `tabSales Invoice` where customer = %(customer)s and docstatus = 1 and name<> %(name)s  """, {"customer":customer,"name":info.name})
			try:	
				saldo_pendiente = "{:.2f}".format(float(saldo_pendiente[0][0]))
			except:
				saldo_pendiente = 0.00
			ruta=frappe.db.get_value("Detalle Ciclo Facturacion",{"sales_invoice":info.name}, "ruta")
			collector=frappe.db.get_value("Rutas de Facturacion", ruta, "collector")
			tel_colector=frappe.db.get_value("Colectores", collector, "telefono")
			if frappe.db.get_value("Customer",customer,"factura_desglosada"):
				planes = frappe.db.sql("""select * from vista_planes_facturas where name = %(name)s;""",{"name": info.name})
			else:
				planes = frappe.db.sql("""select * from vista_planes_facturas where name = %(name)s and descripcion not in ('GponTv22','ITV22');""",{"name": info.name})

			factura = name.partition('-1')

			f.write(f"101|{factura[0]}|{customer_name}{ruc}|{phone}{direccion}|{municipio}|{customer}|{mes}|{posting_date}|{due_date}|{saldo_pendiente}|\n")
			f.write("2\n")
			f.write("3\n")
			paralela = ''
			for plan in range(len(planes)):	
				moneda = "US$"
				nombre = planes[plan][8]
				if not nombre:
					nombre = planes[plan][3]
				head, sep, tail = nombre.partition(' -')
				nombre = head
				currency_item = planes[plan][6]
				paralela = frappe.db.sql(""" select paralela from `tabDetalle Ciclo Facturacion` t1 inner join `tabFacturacion` t2 on t1.parent = t2.name 
							where t1.sales_invoice = %(name)s """, {"name": name})
				try:
					paralela =  float(paralela[0][0])
				except:
					paralela = 1
				if deudores or customer_info[0][5]==1:
					if deudores=='DEUDORES VARIOS - NI' or customer_info[0][5]==1:
						moneda="C$"	
						if currency=="USD":
							amount = "{:.2f}".format(float(planes[plan][5])*paralela)	
							
						else:
							amount = "{:.2f}".format(float(planes[plan][5]))
				else:
					amount = "{:.2f}".format(float(planes[plan][5]))	
				promo =""
				if planes[plan][11]:
					promo = f""" - Aplica promoción "{planes[plan][11]}" """	
				

				if "DELIPOLLO" not in customer_name:
					f.write(f"    1| {nombre}{promo} De {emision} A {facturacion}|{moneda} {amount}|{moneda} {amount}|\n") 
				elif customer == 'SCI-710002504':
					f.write(f"    1| {nombre}{promo} De {emision} A {facturacion}|{moneda} {amount}|{moneda} {amount}|\n") 					
				else:
					f.write(f"    1| {nombre}{promo} De {emision} A {facturacion}|\n")

			if (deudores or customer_info[0][5]==1) and currency == "USD":
				total_n =round((float(total)*paralela),2)
				
			else:
				total_n=total			
			if "DELIPOLLO" in customer_name and customer != 'SCI-710002504':
				f.write(f"    *| Cantidad de Servicios Facturados {len(planes)} De {emision} A {facturacion}| |{moneda} {total_n}\n") 		
				 
							
			"""" fin detalles del item """
			if currency == "USD":
				moneda = "US$"
			elif currency == "NIO":
				moneda = "C$"
			...
			if (deudores or customer_info[0][5]==1) and currency == "USD":
				f.write(f"4   C$ {round((float(total)*paralela),2)}\n")
				f.write(f"5   C$ {round((float(iva)*paralela),2)}\n")
				f.write(f"6   C$ {round((float(grand_total)*paralela),2)}\n")
			else:
				f.write(f"4   {moneda} {total}\n")
				f.write(f"5   {moneda} {iva}\n")
				f.write(f"6   {moneda} {grand_total}\n")
			
			if currency=="USD":
				f.write(f"7   C$ {round((float(grand_total)*paralela),2)}\n")
			elif currency=="NIO":
					base = float(grand_total)/paralela
					base = round(base,2)
					f.write(f"7   US$ {base}\n")
			if collector:
				# f.write(f"8   {collector[0][0]} ({collector[0][1]})\n")
				f.write(f"8   {collector} ({tel_colector})\n")
			else:
				f.write("8   cobranza@ibw.com.ni (22649000)\n")
			f.write("C11|1\nB11\n")
...
def generador_formato_pbs(name):
	...
	try:
		...
		items2 = frappe.db.sql("""select qty,descripcion,rate, amount from `tabSales Invoice Item` where parent = %(parent)s""",{"parent":info.name})
		paralela = frappe.db.sql(""" select paralela from `tabDetalle Ciclo Facturacion` t1 inner join `tabFacturacion` t2 on t1.parent = t2.name 
					where t1.sales_invoice = %(name)s """, {"name": info.name})
		try:
			paralela =  float(paralela[0][0])
		except:
			paralela = float(info.conversion_rate)
		factura = name.partition('-1')
		
		f = open( './ibwni-crm.ibw.com/public/files/output.txt',"w")
		f.write(f"101|{factura[0]}|{customer_name}{ruc}|{phone}{direccion}|{municipio}|{customer}|{mes}|{posting_date}|{due_date}|{saldo_pendiente}|\n")
		f.write("2\n")
		f.write("3\n")

		if info.tipo_factura == 'Prorrateo' or info.tipo_factura == 'Recurrente' or info.tipo_factura == 'Facturacion' or info.tipo_factura == 'Recurrente B':
			for plan in range(len(planes)):	
				moneda = "US$"
				nombre = planes[plan][8]#descripcion del plan
				PlanID = planes[plan][7]#descripcion del plan
				if not nombre:
					nombre = planes[plan][3]#plan name
				head, sep, tail = nombre.partition(' -')
				nombre = head
				currency_item = planes[plan][6]			
				...
				if "DELIPOLLO" not in customer_name :
					f.write(f"    1| {nombre}{promo} De {emision} A {facturacion}|{moneda} {amount}|{moneda} {amount}|\n") 
				elif customer == 'SCI-710002504' and name != 'B-700875' :
					f.write(f"    1| {nombre}{promo} De {emision} A {facturacion}|{moneda} {amount}|{moneda} {amount}|\n") 
				else:
					f.write(f"    1| {nombre}{promo} De {emision} A {facturacion}|\n") 
			...
			if "DELIPOLLO" in customer_name and customer != 'SCI-710002504' and name != 'B-700875':
				f.write(f"    *| Cantidad de Servicios Facturados {len(planes)} De {emision} A {facturacion}| |{moneda} {total_n}\n") 	
			elif name == 'B-700875' :
				f.write(f"    *| Cantidad de Servicios Facturados {len(planes)} De {emision} A {facturacion}| |{moneda} {total_n}\n") 	 
			else:
		...
		elif info.tipo_factura == "Venta de Equipos" or info.tipo_factura == "Prorrateo+":
			for plan in items2:		
				f.write(f"    {int(plan[0])}|{plan[1]}|{moneda} {plan[2]}|{moneda} {plan[3]}|\n") 	
		
		elif info.tipo_factura == "Traslado de Servicio" or info.tipo_factura == "TV Adicional" or info.tipo_factura == "Instalacion OTC" or info.tipo_factura == "Cableado":
			for plan in items2:		
				f.write(f"    1| {plan[1]}|{moneda} {plan[2]}|{moneda} {plan[2]}|\n") 
		else:
			for plan in items2:		
				f.write(f"    1| {plan[1]} De {emision} A {facturacion}|{moneda} {plan[2]}|{moneda} {plan[2]}|\n") 
		...
		if currency=="USD":
			f.write(f"7   C$ {round((float(grand_total)*paralela),2)}\n")
			#f.write(f"7   C$ {base_grand_total}\n")
		...
		file_path  = "ibwni-crm.ibw.com/public/files/output.txt"
		...
		frappe.msgprint(frappe._('generador_formato_pbs {0} ').format(e))
...
@frappe.whitelist()
def generador_de_pbs_todos(name):
	facturas = frappe.db.sql(""" select distinct(ruta) from `tabDetalle Ciclo Facturacion` where parent =%(name)s and ruta is not NULL and sales_invoice is not NULL  order by(ruta); """, {"name":name})
	rutas = []
	contador = 0
	for i in range(len(facturas)):
		rutas.append(facturas[i][0])
	
	for ruta in rutas:
		facts = []
		names = frappe.db.sql(""" select sales_invoice from `tabDetalle Ciclo Facturacion` where parent = %(name)s and ruta = %(ruta)s order by(sales_invoice); """, {"name":name, "ruta":ruta})
		for n in range(len(names)):
			facts.append(names[n][0])
		try:	
			iterador_de_pbs(facts, ruta, name)
		except:
			pass
		contador += 1
		
	frappe.msgprint("hecho")
	# return contador, len(rutas)



@frappe.whitelist()
def generador_de_pbs_todos_por_ruta(ruta,name):
	
	facts = []	
	names = frappe.db.sql(""" select sales_invoice from `tabDetalle Ciclo Facturacion` where parent = %(name)s and ruta = %(ruta)s and cliente not in ('SCI-710001847',
'SCI-710001354',
'SCI-310000005',
'SCI-710002291',
'SCI-710002532',
...
'SCI-710002854',
'SCI-710001440',
'SCI-710001474')  order by(sales_invoice); """, {"name":name, "ruta":ruta})
	for n in range(len(names)):
		facts.append(names[n][0])
	try:	
		iterador_de_pbs(facts, ruta, name)
