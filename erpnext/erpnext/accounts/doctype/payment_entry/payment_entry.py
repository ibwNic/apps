...
class PaymentEntry(AccountsController):
	...
	def on_submit(self):
		if self.difference_amount:
			frappe.msgprint(_("Difference Amount must be zero"))
	...
	def set_missing_values(self):
		if self.payment_type == "Internal Transfer":
			...
			if not self.party_type:
				frappe.msgprint(_("Party Type is mandatory"))

			if not self.party:
				frappe.msgprint(_("Party is mandatory"))
	...
	def validate_payment_type(self):
		if self.payment_type not in ("Receive", "Pay", "Internal Transfer"):
			frappe.msgprint(_("Payment Type must be one of Receive, Pay and Internal Transfer"))

	def validate_party_details(self):
		if self.party:
			if not frappe.db.exists(self.party_type, self.party):
				frappe.msgprint(_("Invalid {0}: {1}").format(self.party_type, self.party))
	...
	def validate_account_type(self, account, account_types):
		...
		# 	frappe.msgprint(_("Account Type for {0} must be {1}").format(account, comma_or(account_types)))
	...
	def validate_mandatory(self):
		for field in ("paid_amount", "received_amount", "source_exchange_rate", "target_exchange_rate"):
			if not self.get(field):
				frappe.msgprint(_("{0} is mandatory").format(self.meta.get_label(field)))
		...
		for d in self.get("references"):
			...
			if d.reference_doctype not in valid_reference_doctypes:
				...
				if not frappe.db.exists(d.reference_doctype, d.reference_name):
					frappe.msgprint(_("{0} {1} does not exist").format(d.reference_doctype, d.reference_name))
				else:
					...
					if ref_doc.docstatus != 1:
						frappe.msgprint(_("{0} {1} must be submitted").format(d.reference_doctype, d.reference_name))
	...
	def update_payment_schedule(self, cancel=0):
		...
		for idx, (key, allocated_amount) in enumerate(invoice_payment_amount_map.items(), 1):
			if not invoice_paid_amount_map.get(key):
				frappe.msgprint(_("Payment term {0} not used in {1}").format(key[0], key[1]))
	...
	def validate_received_amount(self):
		if self.paid_from_account_currency == self.paid_to_account_currency:
			if self.paid_amount < self.received_amount:
				frappe.msgprint(_("Received Amount cannot be greater than Paid Amount"))
	...
	def validate_transaction_reference(self):
		...
		if bank_account_type == "Bank":
			if not self.reference_no or not self.reference_date:
				frappe.msgprint(_("Reference No and Reference Date is mandatory for Bank transaction"))
	...
	def add_tax_gl_entries(self, gl_entries):
		for d in self.get("taxes"):
			...
			if account_currency != self.company_currency:
				frappe.msgprint(_("Currency for {0} must be {1}").format(d.account_head, self.company_currency))
	...
	def add_deductions_gl_entries(self, gl_entries):
		for d in self.get("deductions"):
			if d.amount:
				...
				if account_currency != self.company_currency:
					frappe.msgprint(_("Currency for {0} must be {1}").format(d.account, self.company_currency))
...
def validate_inclusive_tax(tax, doc):
	...
	if cint(getattr(tax, "included_in_paid_amount", None)):
		if tax.charge_type == "Actual":
			...
			frappe.msgprint(_("Valuation type charges can not be marked as Inclusive"))
...
def get_party_details(company, party_type, party, date, cost_center=None):
	...
	if not frappe.db.exists(party_type, party):
		frappe.msgprint(_("Invalid {0}: {1}").format(party_type, party))
...
def get_account_details(account, date, cost_center=None):
	frappe.has_permission("Payment Entry", msgprint=True)
	...
	# except for Payment Entry, only in such case we should msgprint permission error
	if not account_list:
		frappe.msgprint(_("Account: {0} is not permitted under Payment Entry").format(account))
