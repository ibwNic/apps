...
class StockController(AccountsController):
	...
	def validate_qi_presence(self, row):
		...
		if not row.quality_inspection:
			...
			if self.docstatus == 1:
				frappe.msgprint(_(msg), title=_("Inspection Required"), exc=QualityInspectionRequiredError)
			else:
	...
	def validate_qi_submission(self, row):
		...
		if not qa_docstatus == 1:
			...
			if action == "Stop":
				frappe.msgprint(_(msg), title=_("Inspection Submission"), exc=QualityInspectionNotSubmittedError)
			else:
	...
	def validate_qi_rejection(self, row):
		...
		if qa_status == "Rejected":
			...
			if action == "Stop":
				frappe.msgprint(_(msg), title=_("Inspection Rejected"), exc=QualityInspectionRejectedError)
			else:
				frappe.msgprint(_(msg), alert=True, indicator="orange")
	...
	def validate_multi_currency(self):
		if self.currency != self.company_currency:
			frappe.msgprint(_("Internal transfers can only be done in company's default currency"))

	def validate_packed_items(self):
		if self.doctype in ("Sales Invoice", "Delivery Note Item") and self.get("packed_items"):
			frappe.msgprint(_("Packed Items cannot be transferred internally"))
