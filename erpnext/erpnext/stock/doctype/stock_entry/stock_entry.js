...
frappe.ui.form.on('Stock Entry', {
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus===0) {
			...
			frm.add_custom_button(__('Obtener Equipos desde Ordenes de Desinstalacion'), function() {
			...
			frm.add_custom_button(__('Obtener Materiales desde Orden de Presupuesto'), function() {
				var d = new frappe.ui.Dialog({
					...
					fields: [
						{"fieldname":"Orden", "fieldtype":"Link", "label":__("Orden de Presupuesto"), "options":"Service Order", "reqd": "1"},					
						{"fieldname":"fetch", "label":__("Obtener Equipos"), "fieldtype":"Button"}
					]
				});
				//filtrar en cuadro de dialogo				
				d.get_field("Orden").get_query = function(){
					frappe.call({
					...
					var presupuesto = d.get_value("Orden"), filters = {'name': ["in", "(" + localStorage.getItem("filtro")  + ")"]};
					if (presupuesto){ filters['Orden'] = presupuesto; };
