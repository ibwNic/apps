...
from frappe.utils import cint, comma_or, cstr, flt, format_time, formatdate, getdate, nowdate,nowtime
...
class StockEntry(StockController):
	...
	def on_update(self):
		if self.workflow_state == "Validado" and self.stock_entry_type == 'Material Issue':
			equipos = []
...
@frappe.whitelist()
def solicitud_de_equipos_desinstalacion(staff_type, staff_name):	
		
		
	warehouse= frappe.db.get_value("Tecnico", {"name": staff_name},"almacen")

	work_orders = frappe.db.sql(""" select name from `tabService Order`  where tipo_de_orden='DESINSTALACION' and ordered_on_stock= 0 and docstatus=1 and workflow_state="Finalizado"
	and tecnico=%(owner)s 
	union all
	select name from `tabIssue` where tipo_de_orden="Averia" and desinstalacion_equipos= 0  and ordered_on_stock= 1 and docstatus=1 and workflow_state="Finalizado"
	and tecnico=%(owner)s;""", {'owner':staff_name})
	...
	if work_orders:
		...
		equipos = frappe.db.sql(""" select serial_no,item_code,creation,parent from `tabEquipo_Orden_Servicio`  where parent in %(owner)s 
		union all
		select equipo,modelo,creation,parent from `tabIssue_Equipos` where parent in %(owner)s and equipo_nuevo is not null;""", {'owner':lista})
		for row in equipos:				
...
def filtrar_equipos_presupuesto():
	ordenes = frappe.db.sql("""  select so.name from  `tabService Order` so inner join `tabOpportunity` op on so.nombre_de_origen = op.name where so.workflow_state = 'Finalizado' and 
		so.tipo_de_orden = 'PRESUPUESTO' and so.ordered_on_stock = 0 and op.aprobado_por_ventas_corporativas =  1 and op.aprobado_por_gerencia_general = 1; """)
	return [o[0] for o in ordenes]

@frappe.whitelist()
def equipos_de_presupuesto(Orden):
	bom = frappe.db.get_values("Equipos BOM",{"parent":Orden},["item","qty","uom"])
	mr = frappe.new_doc('Stock Entry')
	mr.update({
		'material_request_type': 'Material Issue',
		'posting_date': nowdate(),
