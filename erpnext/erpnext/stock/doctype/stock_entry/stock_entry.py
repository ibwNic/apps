...
from frappe.utils import cint, comma_or, cstr, flt, format_time, formatdate, getdate, nowdate,nowtime,add_to_date
...
class StockEntry(StockController):
	...
	def on_update(self):
		...
		if self.workflow_state == "Validado" and self.stock_entry_type in ['Material Transfer','Material Issue']:
			equipos = []
			for equipo in self.items:
				if equipo.serial_no:
					sn = equipo.serial_no.split("\n")
					equipos.append([sn,equipo.item_code])	
			
			for equipo in equipos:
				for sn in equipo[0]:
					if not sn:
						frappe.msgprint(
							msg='Hay un equipo que no existe en el sistema en el modelo' + equipo[1],
							title="Error",
							indicator='red'
						)
						...
						return
					try:
						if equipo[1] != frappe.db.get_value("Serial No",sn,'item_code'):
							frappe.msgprint(
								msg='El modelo del equipo en el sistema es ' + frappe.db.get_value("Serial No",sn,'item_code'),
								title="El equipo " + sn + " no pertenece al modelo " + equipo[1],
								indicator='red'
							)
							frappe.db.set_value(self.doctype, self.name, 'workflow_state', 'Revisado')
							self.reload()
			...
			for row in self.items:	
				if row.serial_no:				
					if "\n" in row.serial_no:
						...
						for mac in series:
							if ":" in mac and frappe.db.exists("Serial No",{"name": mac.replace(":","")}):
								frappe.msgprint(f"el numero de serie {mac} existe en el sistema de esta forma: <b>{mac.replace(':','')}</b>, favor corregirlo.")
								frappe.db.set_value(self.doctype, self.name, 'workflow_state', 'Guardado')
								self.reload()
								return
							elif ":" not in mac and frappe.db.exists("Serial No",{"name": insert_colons(mac)}):
								frappe.msgprint(f"el numero de serie {mac} existe en el sistema de esta forma: <b>{insert_colons(mac)}</b>, favor corregirlo.")
								frappe.db.set_value(self.doctype, self.name, 'workflow_state', 'Guardado')
								self.reload()
								return
													
					else:
						if ":" in row.serial_no and frappe.db.exists("Serial No",{"name": row.serial_no.replace(":","")}):
							frappe.msgprint(f"el numero de serie {row.serial_no} existe en el sistema de esta forma: <b>{row.serial_no.replace(':','')}</b>, favor corregirlo.")
							frappe.db.set_value(self.doctype, self.name, 'workflow_state', 'Guardado')
							self.reload()
							return
						elif ":" not in row.serial_no and frappe.db.exists("Serial No",{"name": insert_colons(row.serial_no)}):
							frappe.msgprint(f"el numero de serie {row.serial_no} existe en el sistema de esta forma: <b>{insert_colons(row.serial_no)}</b>, favor corregirlo.")
							frappe.db.set_value(self.doctype, self.name, 'workflow_state', 'Guardado')
							self.reload()
							return
		...
		if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
			if frappe.db.get_value("Stock Entry",self.name,"add_to_transit")==0:
				frappe.db.set_value("Stock Entry",self.name,"end_transit",None)
	...
	def on_submit(self):
		...
		if self.tecnico:
			ultimo = frappe.db.sql(""" select MAX(name) from `tabDocShare`;""")
			nuevo = int(ultimo[0][0]) + 1
			usuario=frappe.db.get_value("Tecnico",self.tecnico,"usuario")
			owner=frappe.session.user

			if not usuario:
				usuario=frappe.db.get_value("Tecnico",self.tecnico,"owner")

			if usuario:
				# frappe.db.sql(""" insert into `tabDocShare` (name, creation, modified, modified_by, owner, docstatus, idx, user,     share_doctype, share_name, `read`, `write`, `share`, submit, everyone,notify_by_email)
				# values (%(ultimo)s,now(),now(),%(owner)s,%(owner)s,0,0,%(usuario)s,'Stock Entry',%(name)s,1,1,1,1,0,1);""",
				# {"name":self.name,"ultimo":nuevo,"owner":owner,"usuario":usuario})
		...
		if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
			...
			movimientos = frappe.db.sql("""   select t1.name as 'retorno_transito', t1.outgoing_stock_entry as 'retorno_aceptado',
			(select outgoing_stock_entry from `tabStock Entry` where name = t1.outgoing_stock_entry ) as 'transcerencia_aceptada',
			(select referencia from `tabStock Entry` where name = t1.outgoing_stock_entry ) 'transcerencia_transito'
			from `tabStock Entry` t1 where name = %(name)s;""",{"name":self.name})
			
			retorno_transito = movimientos[0][0]
			try:
				retorno_aceptado =  movimientos[0][1]
			except:
				retorno_aceptado = None
			try:
				transferencia_aceptada = movimientos[0][2]
			except:
				transferencia_aceptada = None
			try:
				transferencia_transito = movimientos[0][3]
			except:
				transferencia_transito = None
			
			if retorno_transito and retorno_aceptado and transferencia_aceptada and transferencia_transito:			
				frappe.db.sql(""" update  `tabStock Entry` set retorno_bodega = 1 where name in (%(retorno_transito)s,
				%(retorno_aceptado)s,%(transferencia_aceptada)s,%(transferencia_transito)s);""",{"retorno_transito":retorno_transito,
				"retorno_aceptado":retorno_aceptado,"transferencia_aceptada":transferencia_aceptada,"transferencia_transito":transferencia_transito})
			#para retorno de bodega 3
			if self.outgoing_stock_entry and self.referencia and not self.end_transit:
				frappe.db.sql(""" update  `tabStock Entry` set retorno_bodega = 1 where name in (%(retorno_transito)s,
				%(transcerencia_aceptada)s,%(salida_bodega)s);""",{"retorno_transito":self.name,
				"transcerencia_aceptada": self.outgoing_stock_entry ,"salida_bodega":self.referencia})

			if not frappe.db.get_value("Stock Entry",self.outgoing_stock_entry,"end_transit") and frappe.db.get_value("Stock Entry",self.outgoing_stock_entry,"add_to_transit")==1:
				frappe.db.set_value("Stock Entry",self.outgoing_stock_entry,"end_transit",self.name)
			if frappe.db.get_value("Stock Entry",self.name,"add_to_transit")==0:
				frappe.db.set_value("Stock Entry",self.name,"end_transit",None)
...
def make_stock_in_entry(source_name, target_doc=None):
	def set_missing_values(source, target):
		...
		target.referencia=source.outgoing_stock_entry
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		# target_doc.tecnico = source_doc.tecnico
...
@frappe.whitelist()
def make_stock_in_entry_return(source_name, target_doc=None):
	def set_missing_values(source, target):
		target.set_stock_entry_type()
		target.set_missing_values()
		target.tecnico=source.tecnico
		...
		target.to_warehouse = "Las mercancías en tránsito - NI"
		#target.retorno = ''
		target.end_transit = ''
		target.add_to_transit=1
	def update_item(source_doc, target_doc, source_parent):
		target_doc.t_warehouse = "Las mercancías en tránsito - NI"
		if source_doc.material_request_item and source_doc.material_request:
			add_to_transit = frappe.db.get_value("Stock Entry", source_name, "add_to_transit")
			if add_to_transit:
				warehouse = frappe.get_value(
...
@frappe.whitelist()
def make_stock_in_entry_return_3(source_name, target_doc=None):
	#frappe.msgprint(source_name)
	first = frappe.get_doc("Stock Entry",source_name)
	second = first.end_transit
	second = frappe.get_doc("Stock Entry", second)
	if second.workflow_state != 'Validado':
		frappe.msgprint("El técnico aún no ha validado la transferencia de materiales recibida")
		return
	def set_missing_values(source, target):
		second = frappe.get_doc("Stock Entry",source.end_transit)
		if second.workflow_state != 'Validado':
			frappe.msgprint("El técnico aún no ha validado la transferencia de materiales recibida")
			return
		target.set_stock_entry_type()
		target.set_missing_values()
		target.tecnico=second.tecnico
		target.referencia=second.outgoing_stock_entry
		target.outgoing_stock_entry = second.name
		target.from_warehouse = second.to_warehouse
		target.to_warehouse = source.from_warehouse
		#target.retorno = ''
		target.end_transit = ''
		target.append('items', {"s_warehouse": second.to_warehouse,"t_warehouse":first.from_warehouse})
	doclist = get_mapped_doc(
		"Stock Entry",
		source_name,
		{
...
def stock_entry_detail_for_issue(lista,staff_name=None):
	""" Recibe como entrada el nombre del técnico y la orden o las ordenes de servicio"""
	...
	query = """select SO.serial_no,(select item_code from `tabSerial No` where name=SO.serial_no)
			item_code,(select warehouse from `tabSerial No` where name=SO.serial_no) warehouse,
			SO.creation,SO.parent from `tabEquipo_Orden_Servicio` as SO where SO.parent in %(owner)s 
			union all
			select equipo_nuevo,modelo_nuevo, (select warehouse from `tabSerial No` where name=equipo_nuevo) warehouse,
			creation,parent from `tabIssue_Equipos` where parent in %(owner)s and LENGTH(equipo_nuevo)>0;"""
	...
	# 		union all
	# 		select equipo_nuevo,modelo_nuevo, (select warehouse from `tabSerial No` where name=equipo_nuevo) warehouse,
	# 		creation,parent from `tabIssue_Equipos` where parent in %(owner)s and equipo_nuevo is not null;"""			
		
	equipos = frappe.db.sql(query, {'owner':lista})

	if len(equipos) > 0:
		for row in equipos:	
			if frappe.db.get_value("Serial No",row[0],"customer"):
				continue
			...
			item1.uom='Unidad'
			item1.stock_uom='Unidad'

	materiales = frappe.db.sql(""" select material,cantidad,parent,bodega,uom,serial_no from `tabMateriales detalles` where parent in %(owner)s ;""", {'owner':lista})
	if materiales:
		for row in materiales:				
			...
			item1.uom=row[4]
			item1.stock_uom=row[4]
			item1.serial_no = row[5]
			item1.item_group=frappe.db.get_value('Item', {"name": row[0]}, 'item_group')
	else:
		return mr
...
@frappe.whitelist()
def almacenes_permitidos(owner):
	#frappe.msgprint(owner)
	almacenes = frappe.db.sql(""" select distinct(t1.name) from `tabWarehouse` t1 inner join
	`tabUser Permission` t2 on t1.name = t2.for_value where t2.user=%(owner)s ;""", {'owner':owner})
	lista =[]
	for i in range(len(almacenes)):
		lista.append(almacenes[i][0])
...
@frappe.whitelist()
def solicitud_de_equipos(staff_type=None, staff_name=None):		
	work_orders = frappe.db.sql(""" select name from `tabService Order` where 								
						ordered_on_stock= 0 and tipo_de_orden not in ('DESINSTALACION','REACTIVACION','SUSPENSION')
						and docstatus=1 and workflow_state="Finalizado" and tipo_de_orden  and tecnico = %(staff_name)s
						and (name in (select distinct parent from `tabMateriales detalles`) or name in (select distinct parent from `tabEquipo_Orden_Servicio`))
						union all
						select name from `tabIssue` where tipo_de_orden in ("Averia", "Ticket Iexpress") and ordered_on_stock= 0 
						and docstatus=1 and workflow_state="Finalizado" and tecnico = %(staff_name)s
						and ( name in (select distinct parent from `tabMateriales detalles`) or name in (select distinct parent from `tabIssue_Equipos` WHERE equipo_nuevo is not null))
						union all
						select name from `tabOrden de Servicio Interno` where ordered_on_stock = 0 and docstatus=1 and workflow_state="Finalizado"
						and name in (select distinct parent from `tabMateriales detalles`) and tecnico = %(staff_name)s
								"""
	...
	if wo:
		...
		materiales = stock_entry_detail_for_issue(lista,staff_name)
		return {'docs': materiales.as_dict()}
...
def expedicion_de_materiales(orden, tecnico):
	lista = []	
	lista.append(orden)
	materiales = stock_entry_detail_for_issue(lista,tecnico)
	try:
		materiales.save(ignore_permissions=True)
		frappe.db.commit()
		materiales.submit()
		return materiales.name
	except:
		frappe.msgprint("No se creó ninguna expedicion de material. Si no hay materiales que liquidar, ignore este mensaje.")



# def recepcion_de_materiales(orden, tecnico):
# 	""" Recibe como entrada el nombre del técnico y la orden o las ordenes de servicio para recepcion"""
	
# 	almacen = frappe.db.sql(""" select almacen from `tabAlmacenes de Tecnico` where almacen like %(usado)s and parent in 
# 		(select tecnico from `tabService Order`where name=%(orden)s ) or 
# 		parent in (select tecnico from `tabTecnicos Service Order`where parent=%(orden)s);""",{"usado":'%usado%',"orden":orden})

# 	mr = frappe.new_doc('Stock Entry')
# 	mr.update({
# 		'material_request_type': 'Material Receipt',
# 		'posting_date': nowdate(),
# 		'posting_time': nowtime(),
# 		'purpose':'Material Receipt',
# 		'stock_entry_type': "Material Receipt",
# 		'tecnico': tecnico,
# 		'to_warehouse': almacen[0][0]
# 	})
# 	lista = [orden]
	
# 	equipos = frappe.db.sql(""" select serial_no,item_code,creation,parent from `tabEquipo_Orden_Servicio` 
# 		where parent in %(owner)s 
# 		union all
# 		select equipo,modelo,creation,parent from `tabIssue_Equipos` where parent in %(owner)s and equipo_nuevo 
# 		is not null;""", {'owner':lista})
...
# 		item1.serial_no = row[0]
# 		item1.qty=1
# 		item1.t_warehouse=almacen[0][0]
# 		item1.uom='Unidad'
# 		item1.stock_uom='Unidad'
# 		item1.allow_zero_valuation_rate = 1

# 		materiales = frappe.db.sql(""" select material,cantidad,parent,bodega,uom, serial_no from `tabMateriales Usados Detalles` where parent in %(owner)s ;""", {'owner':lista})
# 		for row in materiales:							
# 			item1 = mr.append('items', {"serial_no": ""})				
# 			item1.schedule_date = nowdate()
# 			item1.item_code = row[0]
...
@frappe.whitelist()
def solicitud_de_equipos_desinstalacion(staff_name, es_una_orden=False):	
	work_orders = [[]]			
	if not es_una_orden:
		work_orders = frappe.db.sql(""" select name from `tabService Order` where tipo_de_orden in ('DESINSTALACION','DESINSTALACION RCPE' )
		and ordered_on_stock= 0 and docstatus=1 and workflow_state="Finalizado"
		and tecnico=%(owner)s and fecha_finalizado > '2023-08-18 00:00:00.00'
		union all
		select name from `tabIssue` where tipo_de_orden in  ("Averia","Ticket Iexpress") and desinstalacion_equipos= 0  
		and ordered_on_stock= 1 and docstatus=1 and workflow_state="Finalizado"
		and tecnico=%(owner)s and fecha_finalizado>'2023-08-18 00:00:00.00';
		union all
		select name from `tabOrden de Servicio Interno` where desinstalacion_equipos= 0  
		and docstatus=1 and workflow_state="Finalizado"
		and tecnico=%(owner)s and fecha_finalizado>'2023-08-18 00:00:00.00';""", {'owner':staff_name})
	else:
		work_orders[0].append(staff_name)

	lista =[orden[0] for orden in work_orders]
	...
	if work_orders:
		...
		mr.update({
			...
			'purpose':'Material Receipt',
			'stock_entry_type': "Material Receipt",
			'tecnico': staff_name if not es_una_orden else '',
			'to_warehouse': 'Usados - Bodega Central - NI'
		})
		equipos = frappe.db.sql(""" select serial_no,item_code,creation,parent from `tabEquipo_Orden_Servicio` 
		where parent in %(owner)s 
		union all
		...
		for row in equipos:						
			...
			item1.serial_no = row[0]
			item1.qty=1
			item1.t_warehouse='Usados - Bodega Central - NI'
			item1.uom='Unidad'
			item1.stock_uom='Unidad'
			item1.allow_zero_valuation_rate = 1
...
@frappe.whitelist()
def solicitud_de_materiales_segun_portafolio(portafolio,cantidad):
...
@frappe.whitelist()
def filtrar_equipos_presupuesto():
	ordenes = frappe.db.sql("""  select so.name from  `tabService Order` so inner join `tabOpportunity` op on so.nombre_de_origen = op.name where so.workflow_state = 'Finalizado' and 
		so.tipo_de_orden = 'PRESUPUESTO' and so.ordered_on_stock = 0 and op.aprobado_por_ventas_corporativas =  1 and op.aprobado_por_gerencia_general = 1 order by name desc; """)
	return [o[0] for o in ordenes]

@frappe.whitelist()
def filtrar_desintalaciones():
	ordenes = frappe.db.sql("""  select name from `tabService Order` where tipo_de_orden='DESINSTALACION' 
		and ordered_on_stock= 0 and workflow_state="Finalizado" and name in (select distinct parent from `tabMateriales detalles`) or name in (select distinct parent from `tabEquipo_Orden_Servicio`)""")
	return [o[0] for o in ordenes]
...
def filtrar_averias():
	ordenes = frappe.db.sql(""" 
		select name from `tabIssue` where tipo_de_orden in  ("Averia","Ticket Iexpress") and desinstalacion_equipos= 0  
		 and docstatus=1 and workflow_state="Finalizado" and  name in (select distinct parent from `tabMateriales detalles`) or name in (select distinct parent from `tabIssue_Equipos` WHERE equipo_nuevo is not null);""")
	return [o[0] for o in ordenes]

@frappe.whitelist()
def filtrar_od_internas():
	ordenes = frappe.db.sql(""" 
		select name from `tabOrden de Servicio Interno` where desinstalacion_equipos= 0  
		 and docstatus=1 and workflow_state="Finalizado" and  name in (select distinct parent from `tabMateriales Usados Detalles`)""")
	return [o[0] for o in ordenes]



@frappe.whitelist()
def equipos_de_presupuesto(name):
	bom = frappe.db.get_values("Equipos BOM",{"parent":name},["item","qty","uom"])
	mr = frappe.new_doc('Stock Entry')
	mr.update({
		'material_request_type': 'Material Transfer',
		'posting_date': nowdate(),
		...
		'stock_entry_type': "Material Transfer",
		'purpose':"Material Transfer",	
		'from_warehouse': 'Bodega Central - NI'
	})
	for m in bom:
		item_group= frappe.db.get_value('Item', {"name": m[0]}, 'item_group')
		if item_group != 'Instalacion y Configuracion':
			item1 = mr.append('items', {"item_code": ""})				
			item1.schedule_date = nowdate()
			item1.item_code = m[0]
			item1.qty = int(m[1])
			item1.uom=frappe.db.get_value('Item', {"name":m[0]}, 'stock_uom')
			item1.item_group=frappe.db.get_value('Item', {"name": m[0]}, 'item_group')
			item1.stock_uom=frappe.db.get_value('Item', {"name": m[0]}, 'stock_uom')

			
			item1.service_order = name
			item1.s_warehouse = mr.from_warehouse
	return {'docs': mr.as_dict()}