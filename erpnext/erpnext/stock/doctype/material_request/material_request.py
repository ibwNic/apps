...
def update_completed_and_requested_qty(stock_entry, method):
	...
	if stock_entry.doctype == "Stock Entry":
		...
		equipos = frappe.db.sql(""" select count(service_order) from `tabStock Entry Detail` where parent = %(parent)s """, {"parent":stock_entry.name})
		equipos = equipos[0][0]
		ordenes = frappe.db.sql(""" select service_order, serial_no from `tabStock Entry Detail` where parent = %(parent)s  and serial_no is not null and serial_no <> '' and docstatus=1;""", {"parent":stock_entry.name})
		customer = ''
		materiales = frappe.db.sql(""" select service_order, item_code, qty, uom, valuation_rate, amount from `tabStock Entry Detail` where parent =  %(parent)s and serial_no is null or serial_no = '' and docstatus=1;""", {"parent":stock_entry.name})
		if method == "on_submit" and stock_entry.stock_entry_type == "Material Issue" and equipos >0:
			...
			for orden in range(len(ordenes)):
				...
				if head == "OS":
					...
					frappe.db.set_value('Serial No', issue[0][0], 'customer', None)
					frappe.db.set_value('Issue',ordenes[orden][0], 'ordered_on_stock', 1)
					...
					if frappe.db.get_value('Subscription Plan Equipos', {"parent":suscripcion,"equipo":orden_issue[0][0]}):
						frappe.get_doc("Subscription Plan Equipos",{"parent":suscripcion, "equipo":orden_issue[0][0]}).delete()
			...
			if materiales:
				for orden in materiales:
					...
					add_to_bitacora_material.insert()
		...
		if method == "on_submit" and equipos == 0  and stock_entry.stock_entry_type == "Material Transfer":
		...
		if method == "on_cancel" and equipos>0 and stock_entry.stock_entry_type == "Material Issue":
			for orden in range(len(ordenes)):	
				...
				if head == "OS":
					...
					frappe.db.set_value('Issue',ordenes[orden][0], 'ordered_on_stock', 0)					
					frappe.db.set_value('Serial No', ordenes[orden][1], 'customer', None)
					if frappe.db.get_value('Subscription Plan Equipos', {"parent":suscripcion,"equipo":ordenes[orden][1]}):
						frappe.get_doc("Subscription Plan Equipos",{"parent":suscripcion, "equipo":ordenes[orden][1]}).delete()
						add_equipos = frappe.get_doc({
