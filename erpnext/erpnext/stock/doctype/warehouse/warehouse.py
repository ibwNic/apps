...
from frappe.utils import random_string, now, today
...
@frappe.whitelist()
def permiso_De_Almacen(docname):
	if not frappe.db.exists("User Permission",{"for_value":docname,"usuario":frappe.session.user}):
...
# 		frappe.db.sql(""" insert into `tabUser Permission` 
# 			(name,creation,modified,modified_by,owner,user,allow,for_value,apply_to_all_doctypes) values 
# 			(%(name)s,%(fecha)s, %(fecha)s, %(usuario)s,%(usuario)s,%(usuario)s,'Warehouse',%(docname)s,1)""",{"name":random_string(8),"usuario":frappe.session.user, "fecha":now(),"docname":docname })
