...
def validate_serial_no(sle):
	...
	for sn in get_serial_nos(sle.serial_no):
		...
		if vouchers:
			...
			try:
				frappe.msgprint(_(msg), title=_(title), exc=SerialNoExistsInFutureTransaction)
			except:
				pass
...
def validate_item_warehouse(args):
	for field in ["item_code", "warehouse", "posting_date", "posting_time"]:
		if not args.get(field):
			...
			frappe.msgprint(_(validation_msg))
...
class update_entries_after(object):
	...
	def __init__(
		self,
		args,
		# originalmente es allow_zero_rate=False,
	...
	def raise_exceptions(self):
		...
		if msg_list:
			message = "\n\n".join(msg_list)
			if self.verbose:
				frappe.msgprint(str(message))#, NegativeStockError, title=_("Insufficient Stock"))
...
def get_valuation_rate(
	...
	if (
		...
		frappe.msgprint(msg=msg, title=_("Valuation Rate Missing"))
...
def validate_negative_qty_in_future_sle(args, allow_negative_stock=False):
	...
	if is_negative_with_precision(neg_sle):
		...
		#frappe.msgprint(message, NegativeStockError, title=_("Insufficient Stock"))
		frappe.throw(_("{0}").format(message), NegativeStockError)
	...
	if is_negative_with_precision(neg_batch_sle, is_batch=True):
		...
		frappe.msgprint(message, NegativeStockError, title=_("Insufficient Stock for Batch"))
