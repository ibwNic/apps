...
class Lead(SellingController, CRMNote):
	...
	def on_cancel(self):
		frappe.db.sql(""" update `tabLead` set workflow_state = 'Cancelado',docstatus = 1 where name = %(name)s; """,{"name":self.name})
		self.reload()
		return 	

	def on_update(self):
		if self.workflow_state in ["Aprobado con Deposito","Aprobado"]:
			try:
				direccion = frappe.db.sql(""" select address_line1 from `tabAddress` where name in (select parent from `tabDynamic Link` 
				where link_doctype = 'Lead' and link_name =%(name)s and parenttype = 'Address') limit 1;""", {"name":self.name})[0][0]
			except:
				direccion = None
			if not direccion:
				frappe.msgprint(
				...
				frappe.db.set_value(self.doctype, self.name, 'workflow_state', 'Esperando Validacion')
				frappe.db.set_value(self.doctype, self.name, 'docstatus', 0)
				self.reload()
				return 
			
			phones = frappe.db.sql(""" select phone, mobile_no from `tabContact` where name in (select parent from `tabDynamic Link` 
				where link_doctype = 'Lead' and link_name =%(name)s and parenttype = 'Contact')""",{"name":self.name})
			try:
				phones = phones[0][0] or phones[0][1]
			except:
				phones = None
			if not phones:
				frappe.msgprint(
	...
	def validate_Cedula(self):
		if self.cedula:
			if len(self.cedula) == 16:
				pass
				# if not self.cedula[0:3].isdigit():
				# 	frappe.throw(_("Numero de Cédula invalida, ingrese un numero de cedula correcto"))
				
				# if self.cedula[3:4] != '-':
				# 	frappe.throw(_("Numero de Cédula invalida, ingrese un numero de cedula correcto"))
				
				# if not self.cedula[4:10].isdigit():
				# 	frappe.throw(_("Numero de Cédula invalida, ingrese un numero de cedula correcto"))

				# if self.cedula[10:11] != '-':
				# 	frappe.throw(_("Numero de Cédula invalida, ingrese un numero de cedula correcto"))

				# if not self.cedula[11:15].isdigit():
				# 	frappe.throw(_("Numero de Cédula invalida, ingrese un numero de cedula correcto"))
					
				# if self.cedula[15:16].isdigit and not self.cedula[15:16].isalpha():
				# 	frappe.throw(_("Numero de Cédula invalida, ingrese un numero de cedula correcto"))
			else:
				if self.pasaporte:
					pass
				else:
					frappe.throw(_("Numero de cedula invalido, no se permite menos o más de 16 caracteres"))   
...
@frappe.whitelist()
def get_bitacora_lead(cedula):
	
	if len(cedula)>1:
		cedulas = 	frappe.db.sql(""" SELECT name  FROM  `tabLead` l where l.cedula = %(cedula)s
				union all SELECT name FROM  `tabCustomer` c where c.cedula = %(cedula)s;""",{"cedula":cedula})		
...
def verificar_cedula(cedula):
	...
	if len(cedula)>1:
		...
		try:
			...
			lista =[cedula[0] for cedula in cedulas]
			if len(lista)>0:		
				frappe.msgprint("Esa cédula ya existe en los siguientes documentos: "+ str(lista) )
				return True
		except:
			return len(cedula)
	else:
		return len(cedula)
@frappe.whitelist()
def verificar_nis(nis):
	# cedula=cedula.replace(" ","")
	if len(nis)>1:
		names = 	frappe.db.sql(""" SELECT name  FROM  `tabLead` l where l.nis = %(nis)s
				union all SELECT name FROM  `tabCustomer` c where c.nis = %(nis)s;""",{"nis":nis})
		
		try:
			# cedulas = cedulas[0]

			lista =[name[0] for name in names]
			if len(lista)>0:		
				frappe.msgprint("Este Nis ya existe en los siguientes documentos: "+ str(lista) )
				return True
		except:
			return False
...
@frappe.whitelist()
def verificar_cedula_html(cedula):
	# cedula=cedula.replace(" ","")
	if len(cedula)>1:
		lista_cedula = 	frappe.db.sql(""" SELECT name,"lead" as tipo  FROM  `tabLead` l where l.cedula = %(cedula)s
				union all SELECT name,"customer"as tipo FROM  `tabCustomer` c where c.cedula = %(cedula)s;""",{"cedula":cedula})
		
		return lista_cedula	

@frappe.whitelist()
def verificar_nis_html(nis):
	# cedula=cedula.replace(" ","")
	if len(nis)>1:
		lista_nis = 	frappe.db.sql(""" SELECT name,"lead" as tipo   FROM  `tabLead` l where l.nis = %(nis)s
				union all SELECT name,"customer"as tipo FROM  `tabCustomer` c where c.nis = %(nis)s;""",{"nis":nis})
